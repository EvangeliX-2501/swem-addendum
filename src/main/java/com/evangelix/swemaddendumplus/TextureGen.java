...
public class TextureGen {
    ...
    public static ResourceLocation getRandomClientCoat(String folderName) {
        ...
        for(Pack pack: packRepository.getSelectedPacks()) {
            ...
            SwemAddendumPlusMain.LOGGER.debug("Looking for coats in: " + pack.getId());
            ...
            try(PackResources packResources = pack.open()) {
                ...
                SwemAddendumPlusMain.LOGGER.error("Failed to randomly get a client coat from: " + folderName);
    ...
    public static ResourceLocation getRandomServerCoat(String folderName) {
        ...
        for(Pack pack : packRepository.getSelectedPacks()) {
            ...
            try(PackResources packResources = pack.open()) {
                ...
                SwemAddendumPlusMain.LOGGER.error("Failed to randomly get server coat from: " + folderName);
                return MissingTextureAtlasSprite.getLocation();
            }
        }
        if(allTextures.size() == 0) {
            SwemAddendumPlusMain.LOGGER.error("Failed to randomly get a server coat from: " + folderName);
            return MissingTextureAtlasSprite.getLocation();
        }
        return allTextures.get(random.nextInt(allTextures.size()));
    }
    ...
    public static List<ResourceLocation> getAllCoats(String folderName) {
        ...
        for(Pack pack : packRepository.getSelectedPacks()) {
            try(PackResources packResources = pack.open()) {
                ...
                SwemAddendumPlusMain.LOGGER.error("Failed getting coat data from " + pack.getId() + ": " + e);
            }
        }
        return allTextures;
    }
